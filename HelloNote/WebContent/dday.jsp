...
<script>
	$(function() {
		...
		$('#ddayTable').on('click', 'tr td:nth-child(1) span', function() {
			...
			// 			$tr.children().eq(0).append('<input type="text" id="ddayNameEdit" value="'+$tr.children().eq(0).text()+'">');
		});
				...
				.on(
						...
						function() {
							...
							$('#ddayTable')
									.append(
											'<tr id="dday'+i
											+'"><td width="1%"></td><td><input style="width: 100%;" type="text" class="ddayInput" id="ddayNameInput"></td><td style="width: 10%;" class="theDay"><input type="date" id="ddayDateInput"></td></tr>');
							$('#add').hide();
							$('#confirm').show();
						});
				...
				.on(
						...
						function() {
							...
							$('#dday' + i).children().eq(1).append(
									'<span id="ddayName'+i+'">' + inputName
											+ '</span>');
							// 입력 날짜가 현재 날짜 전 이면
							if (gap > 0) {
								$('#dday' + i).children().eq(2).append(
										'<span id="ddayDate'+i+'">D+' + gap
												+ '</span>');
							}
							// 입력 날짜가 현재 날짜면
							else if (gap == 0) {
...
<body>
	<div id="mainContainer" align="center">
		<button id="delete" style="margin-left: 75%;">삭제</button>
